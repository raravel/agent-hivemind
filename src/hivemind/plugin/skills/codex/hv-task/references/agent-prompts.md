# Worker Prompt Templates -- Orchestrator Model

Prompt templates for the orchestrator pipeline. The orchestrator delegates coding and review to subagents via `spawn_agent`, but runs tests and makes all verification/completion decisions itself.
...
| Coding Worker | Subagent worker | `executor` from profile | Implements code changes |
| Review Worker | Subagent worker | `reviewer` from profile | Produces review feedback |

## Coding Worker Prompt Template

Use this template when spawning the coding worker via `spawn_agent`:
...
Use this template when spawning the review worker via `spawn_agent`:
...
These are NOT `spawn_agent` prompts — they are internal reasoning steps the orchestrator performs.
...
5. Outputs:
   ...
   ```
6. If any `[FAIL]`: send specific details to the worker via `send_input`.
...
When a worker needs to fix something, use **send_input** (not a fresh `spawn_agent` call) to continue the same worker. This preserves the worker's context so it can fix efficiently:

```
send_input to coding worker:
