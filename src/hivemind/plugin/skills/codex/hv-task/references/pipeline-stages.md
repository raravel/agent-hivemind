...
The `hv-task` pipeline uses an **orchestrator model**: the main context (orchestrator) delegates work to subagents but verifies results directly before proceeding.
...
**Actor:** Coding Worker (via `spawn_agent`, **executor** model)
...
5. If any criterion is `[FAIL]`:
   - Use **send_input** to continue the same worker with specific failure details.
   - Max 2 retries.
...
3. If new failures exist:
   - Use **send_input** to send test output to the coding worker.
...
**Actor:** Review Worker (via `spawn_agent`, **reviewer** model)
...
3. If blocking issues exist:
   - Use **send_input** to send feedback to the coding worker.
...
|-------|-------------|--------|---------------|
| VERIFY_CODE | 2 | `send_input` to coding worker | `blocked` |
| RUN_TESTS | 2 | `send_input` to coding worker | `blocked` |
| JUDGE_REVIEW | 1 | `send_input` to coding worker | `blocked` |
