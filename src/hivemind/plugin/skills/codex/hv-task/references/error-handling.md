...
When an error occurs during the orchestrator pipeline, the orchestrator handles retries via **send_input** (continuing the same worker to preserve context) rather than spawning fresh workers.
...
- **Recovery:** Use `send_input` to continue the worker, or spawn a fresh worker if the worker is unresponsive.
...
- **Recovery:** Use `send_input` to the worker with specific failure details.

## Retry Policy

| Stage | Max Retries | Method | What Orchestrator Sends |
|-------|-------------|--------|------------------------|
| VERIFY_CODE | 2 | `send_input` to coding worker | Specific `[FAIL]` criteria with file/line details |
| RUN_TESTS | 2 | `send_input` to coding worker | Full test output (stdout + stderr) |
| JUDGE_REVIEW | 1 | `send_input` to coding worker | Blocking issues from review |

### Why `send_input` over spawning a fresh worker
...
Failure detected by orchestrator
  │
  ├── Retry 1: send_input with specific failure details
  │   └── Orchestrator re-verifies
  │       ├── Pass → continue pipeline
  │       └── Fail → Retry 2
  │
  ├── Retry 2: send_input with updated failure details
  │   └── Orchestrator re-verifies
  │       ├── Pass → continue pipeline
  │       └── Fail → Escalate
  │
