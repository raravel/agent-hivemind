...
- **Force sequential mode** for this run, regardless of `--parallel` or `parallel.max_concurrency`. Codex companion's session-resume scope is per-repo, so concurrent worktrees can collide. Print one line: `"codex provider routing detected — running sequentially"`.
- The codex-plugin-cc plugin (`codex:codex-rescue` subagent) must be installed. If `Agent(subagent_type:"codex:codex-rescue", ...)` later returns "no such subagent", stop and tell the user to run `/codex:setup`.
...
**Protocol (codex provider — uses two Agent calls with codex session resume)**:

When `coder_provider == codex`, the Claude `Agent` call cannot continue the codex session via SendMessage (the rescue subagent is a one-shot forwarder). Use codex's own resume mechanism instead.
...
1. **Do NOT call `codex-companion.mjs` directly via Bash.** That script lives inside the codex-plugin-cc plugin, not the hv plugin, so any `${CLAUDE_PLUGIN_ROOT}/scripts/...` path resolves wrong from this skill. Always go through `Agent(subagent_type:"codex:codex-rescue", ...)` and let the rescue forwarder handle codex internals. The orchestrator does not need a resume precheck — Step A always uses `--fresh` (clean slate), and Step B's `--resume` resumes the most recent codex session in the repo, which is the Step A session we just created.
2. **Step A** — spawn `Agent(subagent_type:"codex:codex-rescue", isolation:"worktree", description:"Step A <TASK-ID>", prompt:<see below>)`. Prompt MUST contain `--fresh` on its own line at the top so the rescue forwarder routes it as a new codex thread:
   ```
   --fresh
...
5. **Step B** — spawn `Agent(subagent_type:"codex:codex-rescue", isolation:"worktree", description:"Step B <TASK-ID>", prompt:<see below>)`. Prompt MUST start with `--resume` so the rescue forwarder adds `--resume-last`:
   ```
   --resume
...
Continue to step 7. From here on, every codex coding call uses `--resume` (see step 7 codex variant).
...
**Record usage**: when the Agent call returns, note the response length and your prompt length — you'll estimate Claude tokens in step 13. **Codex token usage is opaque to you** — record only what you can see (your prompt + response chars) and mark `tokens.codex_estimated: false` in the report.
...
3. **Check completion criteria**: for each `- [ ]` in the task body, output a verification line:
   ...
   ```
4. **On any [FAIL]**:
   - Claude provider: SendMessage to the same worker with the failed criteria.
   - Codex provider: spawn a fresh `Agent(subagent_type:"codex:codex-rescue", isolation:"worktree", ...)` whose prompt starts with `--resume` so the rescue forwarder issues `codex task --resume-last`. Include the failed criteria verbatim. Each retry is a separate Agent call but lands in the same codex thread.
...
1. Send the failing output to the coding worker:
   - Claude provider: SendMessage to the same worker.
   - Codex provider: spawn `Agent(subagent_type:"codex:codex-rescue", isolation:"worktree", ...)` with prompt starting `--resume` and the failing output appended.
...
Wait for the worker to return. The rubric scores you extract in step 11 may show more variance than Claude reviewers — that is expected for the verification trial.
...
1. Send the review output to the coding worker:
   - Claude provider: SendMessage to the same worker.
   - Codex provider: spawn `Agent(subagent_type:"codex:codex-rescue", isolation:"worktree", ...)` with prompt starting `--resume` and the review output appended.
...
- **ALWAYS** use **SendMessage** to continue a Claude worker (preserves worktree + context). For codex workers, use a fresh `Agent(subagent_type:"codex:codex-rescue", ...)` call with prompt starting `--resume` — the codex companion handles `--resume-last` routing.
