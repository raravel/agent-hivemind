...
3. On any unmatched path: SendMessage to the worker —
...
4. If the worker requested `hv task scope-add`, YOU (the orchestrator) must verify the new scope entry is disjoint from every in-flight worker's declared scope (use `hv task get <id>` per peer + `core.scope.conflicts`). On any conflict, SendMessage:
...
3. For each task, spawn Agent with both `run_in_background: true` AND `isolation: "worktree"`.
