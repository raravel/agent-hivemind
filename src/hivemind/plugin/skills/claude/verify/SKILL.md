...
- User runs `/hv:verify` or `/hv:verify <stage>` explicitly
...
If neither exists, stop and tell the user to run `/hv:create-verify` first.
...
- **Arg given** (e.g. `/hv:verify test`): run only that stage. If the stage name does not exist in verify.md, list the available stages and stop.
...
- `lint` failed → "Fix the reported issues, then re-run `/hv:verify lint`."
